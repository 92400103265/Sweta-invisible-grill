import { getServiceRoute } from '@/data/servicesData';

export interface SearchDataItem {
  id: string;
  name: string;
  category: string;
  description: string;
  keywords: string;
  useCase: string;
  locations: string;
  url: string;
}

export const SEARCH_DATA: SearchDataItem[] = [
  {
    id: "invisible-grills",
    name: "Invisible Grills",
    category: "Invisible Grills",
    description:
      "Premium invisible grills installation for balcony and windows. Marine-grade SS316, rust-proof, child-safe, 10-year warranty.",
    keywords:
      "invisible grills marine-grade SS316 rust-proof child-safe unobstructed views balcony window security",
    useCase: "Balcony windows security aesthetics",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/invisible-grills",
  },
  {
    id: "invisible-grills-balcony",
    name: "Invisible Grills for Balcony",
    category: "Invisible Grills",
    description:
      "Secure balcony protection with elegant invisible grills using high quality steel cables.",
    keywords:
      "balcony invisible grills high quality steel cables modern aesthetics premium protection",
    useCase: "Balcony protection family safety property value",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/invisible-grills-balcony",
  },
  {
    id: "invisible-grills-dealer",
    name: "Invisible Grills Dealer",
    category: "Invisible Grills",
    description:
      "Authorized dealership with wholesale pricing, territory rights, training and support.",
    keywords:
      "dealer wholesale bulk orders territory rights training support invisible grills",
    useCase: "Business partnership hardware stores contractors",
    locations: "Gurugram Delhi NCR Noida",
    url: getServiceRoute("invisible-grills-dealer"),
  },
  {
    id: "balcony-safety",
    name: "Balcony Safety Nets",
    category: "Safety Nets",
    description: "Transparent balcony protection. UV-resistant, weather-proof HDPE nets.",
    keywords:
      "balcony safety nets UV-resistant weather-proof HDPE transparent child protection",
    useCase: "Apartment balcony family safety child protection",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/balcony-safety",
  },
  {
    id: "children-protection",
    name: "Children Protection Nets",
    category: "Safety Nets",
    description: "Child-safe HDPE protective nets. Bite-resistant and durable.",
    keywords:
      "children protection child safety nets HDPE bite-resistant durable kids",
    useCase: "Windows balcony playgrounds child safety",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/children-protection",
  },
  {
    id: "pets-safety",
    name: "Pets Safety Nets",
    category: "Safety Nets",
    description: "Pet-friendly, bite-resistant, non-toxic nets for balcony and terraces.",
    keywords:
      "pet safety nets pet-friendly bite-resistant non-toxic pet accident prevention",
    useCase: "Balcony terraces pet containment",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/pets-safety",
  },
  {
    id: "grill-balcony",
    name: "Grill Balcony Safety Nets",
    category: "Safety Nets",
    description:
      "Heat-resistant, fire-retardant, grease-resistant nets for balcony grilling areas.",
    keywords:
      "grill balcony nets heat-resistant fire-retardant grease-resistant outdoor cooking",
    useCase: "Balcony grilling outdoor cooking safety",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/grill-balcony",
  },
  {
    id: "terrace-top",
    name: "Terrace Top Nets",
    category: "Safety Nets",
    description: "UV protection, rain resistant, wind resistant nets for terraces and outdoor living.",
    keywords:
      "terrace nets UV protection rain resistant privacy coverage wind resistant shade",
    useCase: "Terraces outdoor living shade control",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/terrace-top",
  },
  {
    id: "industrial-safety",
    name: "Industrial Safety Nets",
    category: "Safety Nets",
    description: "Fire-retardant OSHA-compliant safety nets for warehouses and factories.",
    keywords:
      "industrial safety nets fire-retardant OSHA compliant worker protection heavy-duty",
    useCase: "Warehouses factories construction sites",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/industrial-safety",
  },
  {
    id: "duct-area",
    name: "Duct Area Nets",
    category: "Safety Nets",
    description: "Fire-retardant ventilation-safe nets for HVAC ducts and utility spaces.",
    keywords:
      "duct area nets fire-retardant ventilation safe debris prevention bird prevention HVAC",
    useCase: "HVAC ducts ventilation shafts utility spaces",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/duct-area",
  },
  {
    id: "open-area",
    name: "Open Area Safety Nets",
    category: "Safety Nets",
    description:
      "UV stabilized large coverage nets for gardens, playgrounds and recreational areas.",
    keywords:
      "open area nets UV stabilized playground safety garden protection large coverage",
    useCase: "Gardens playgrounds recreational areas",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/open-area",
  },
  {
    id: "staircase-safety",
    name: "Staircase Safety Nets",
    category: "Safety Nets",
    description: "Fall prevention custom-fitted nets for indoor and outdoor staircases.",
    keywords:
      "staircase nets fall prevention child-safe custom-fitted aesthetic design elderly safety",
    useCase: "Indoor outdoor stairs elderly safety",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/staircase-safety",
  },
  {
    id: "construction-safety",
    name: "Construction Safety Nets",
    category: "Safety Nets",
    description:
      "OSHA compliant heavy-duty worker and debris protection nets for construction sites.",
    keywords:
      "construction safety nets OSHA compliant worker protection debris control heavy-duty",
    useCase: "Construction sites worker safety",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/construction-safety",
  },
  {
    id: "mosquito-nets",
    name: "Mosquito Nets",
    category: "Safety Nets",
    description:
      "Fine mesh insect protection for doors and windows. Dengue and malaria prevention.",
    keywords:
      "mosquito nets fine mesh dengue prevention malaria protection ventilation friendly",
    useCase: "Doors windows insect prevention",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/mosquito-nets",
  },
  {
    id: "cloth-drying",
    name: "Ceiling Cloth Drying Hangers",
    category: "Cloth Hangers",
    description: "Space-saving ceiling-mounted pulley clothesline for apartments.",
    keywords:
      "cloth drying hangers ceiling mounted space-saving pulley system rust-proof hanger hangers balcony cloth hangers ceiling hanger",
    useCase: "Apartments homes indoor drying",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/cloth-drying",
  },
  {
    id: "hdpe-nylon",
    name: "HDPE Nylon Nets",
    category: "Safety Nets",
    description:
      "Versatile high-tensile weather-proof HDPE and nylon nets with custom specifications.",
    keywords:
      "HDPE nets nylon nets versatile high-tensile weather-proof custom specifications multipurpose",
    useCase: "Multiple applications multipurpose use",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/hdpe-nylon",
  },
  {
    id: "pigeon-nets",
    name: "Pigeon Nets",
    category: "Bird Protection",
    description:
      "Humane pigeon control. Weather-resistant netting to prevent bird nesting and mess.",
    keywords:
      "pigeon nets humane bird control bird nesting prevention mess prevention weather-resistant",
    useCase: "Balcony residential pigeon prevention",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/pigeon-nets",
  },
  {
    id: "bird-spikes",
    name: "Bird Spikes",
    category: "Bird Protection",
    description:
      "Stainless steel 304 bird deterrent spikes. Humane, low-maintenance, for ledges and signs.",
    keywords:
      "bird spikes humane deterrent perching prevention stainless steel 304 low-maintenance",
    useCase: "Ledges signs architectural features",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/bird-spikes",
  },
  {
    id: "anti-bird-nets",
    name: "Anti Bird Nets",
    category: "Bird Protection",
    description: "Comprehensive multi-species humane bird control for large areas.",
    keywords:
      "anti-bird nets comprehensive bird control multi-species protection humane solution",
    useCase: "Large areas combined bird control",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/anti-bird-nets",
  },
  {
    id: "pigeon-balcony",
    name: "Pigeon Nets For Balcony",
    category: "Bird Protection",
    description: "Custom-fitted low-profile pigeon nets for residential balcony.",
    keywords:
      "balcony pigeon nets custom-fitted aesthetic design bird control low-profile residential",
    useCase: "Residential balcony bird prevention",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/pigeon-balcony",
  },
  {
    id: "anti-seagull",
    name: "Anti Seagull Nets",
    category: "Bird Protection",
    description:
      "Marine-grade heavy-duty coastal bird protection nets. Salt-water resistant.",
    keywords:
      "anti-seagull nets marine-grade coastal environments heavy-duty salt-water resistant",
    useCase: "Coastal properties commercial buildings",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/anti-seagull",
  },
  {
    id: "all-sports-practice",
    name: "All Sports Practice Nets",
    category: "Sports Nets",
    description:
      "Multi-purpose professional-grade nets for cricket, football, tennis, badminton, volleyball.",
    keywords:
      "all-sports nets cricket football tennis badminton volleyball professional-grade multi-sport",
    useCase: "Training facilities multi-sport use",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/all-sports-practice",
  },
  {
    id: "cricket-practice",
    name: "Cricket Practice Nets",
    category: "Sports Nets",
    description: "Professional cricket training nets for batting and bowling practice.",
    keywords:
      "cricket nets cricket practice nets cricket net installation best cricket nets coaching ball practice bowling training batting practice",
    useCase: "Cricket nets cricket academies practice grounds schools",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/cricket-practice",
  },
  {
    id: "terrace-cricket",
    name: "Terrace Cricket Nets & Box Cricket Nets",
    category: "Sports Nets",
    description:
      "Rooftop and compact space cricket nets. Portable and custom-fitted for terrace cricket.",
    keywords:
      "terrace cricket nets box cricket nets cricket turf installation cricket turf construction rooftop cricket portable custom cricket nets turf cricket turf nets box cricket construction",
    useCase: "Rooftops apartments terrace cricket turf cricket cricket turf installation construction",
    locations: "Gurugram Delhi NCR Noida",
    url: "/services/terrace-cricket",
  },
];

export const FUSE_OPTIONS = {
  includeScore: true,
  threshold: 0.2, // Stricter: 0.2 = 80% match required (lower = stricter)
  ignoreLocation: false, // Use positional matching for better prefix matching
  minMatchCharLength: 2, // Require at least 2 chars to match
  keys: [
    { name: "name", weight: 3.0 }, // Highest priority: exact service names
    { name: "category", weight: 2.0 }, // Second priority: category names
    { name: "keywords", weight: 1.5 }, // Tags/keywords
    { name: "useCase", weight: 0.8 }, // Use cases
    { name: "locations", weight: 0.5 }, // Locations (lower priority in fuzzy)
    { name: "description", weight: 0.3 }, // Lowest priority
  ],
};

// Expand common query synonyms so short terms like "turf" or "cricket turf" resolve reliably.
export const normalizeSearchQuery = (query: string): string => {
  const normalized = query.toLowerCase().trim();
  if (!normalized) return normalized;

  // "cricket turf installation/construction/nets" → terrace cricket
  if (/cricket.*(turf|construction|installation)|turf.*(cricket|nets?)/.test(normalized)) {
    return `${normalized} terrace cricket box cricket`;
  }

  // bare "turf" → terrace cricket
  if (/\bturf\b/.test(normalized)) {
    return `${normalized} terrace cricket box cricket`;
  }

  // "cricket nets" without "practice" or "terrace" → cricket practice nets
  if (/\bcricket\s+nets?\b/.test(normalized) && !/practice|terrace|box|turf/.test(normalized)) {
    return `${normalized} cricket practice`;
  }

  return normalized;
};

// Location names (only source of truth for location matching)
const LOCATION_NAMES = ["gurugram", "delhi-ncr", "noida"] as const;

// Export for reference (dynamic - no hardcoded variations)
export const LOCATION_KEYWORDS = {
  gurugram: ["gurugram", "gurgaon"],
  "delhi-ncr": ["delhi", "ncr", "delhi-ncr"],
  noida: ["noida", "greater-noida"],
} as Record<string, string[]>;

// Memoization cache for Levenshtein distance calculations
const levenshteinCache = new Map<string, number>();

// Helper to create cache key
const getCacheKey = (a: string, b: string): string => `${a}|${b}`;

// Calculate Levenshtein distance with memoization (optimized for performance)
const levenshteinDistance = (a: string, b: string): number => {
  // Quick exit for identical strings
  if (a === b) return 0;
  
  const cacheKey = getCacheKey(a, b);
  if (levenshteinCache.has(cacheKey)) {
    return levenshteinCache.get(cacheKey)!;
  }

  const [shorter, longer] = a.length < b.length ? [a, b] : [b, a];
  
  if (longer.length === 0) {
    levenshteinCache.set(cacheKey, shorter.length);
    return shorter.length;
  }

  const editDistances: number[] = Array(shorter.length + 1)
    .fill(null)
    .map((_, i) => i);

  for (let i = 1; i <= longer.length; i++) {
    let lastDiagonalValue = i - 1;
    for (let j = 1; j <= shorter.length; j++) {
      const oldDiagonalValue = editDistances[j];
      const cost = longer[i - 1] === shorter[j - 1] ? 0 : 1;
      editDistances[j] = Math.min(
        editDistances[j] + 1,
        editDistances[j - 1] + 1,
        lastDiagonalValue + cost
      );
      lastDiagonalValue = oldDiagonalValue;
    }
  }

  const distance = editDistances[shorter.length];
  levenshteinCache.set(cacheKey, distance);
  return distance;
};

// Helper: Extract location from search query using Levenshtein distance (fuzzy matching)
export const extractLocationFromQuery = (query: string): string | null => {
  const lowerQuery = query.toLowerCase().trim();
  
  if (!lowerQuery) {
    return null;
  }

  // Split query into words and check each word against location names
  const words = lowerQuery.split(/\s+/);
  let bestMatch: string | null = null;
  let bestDistance = Infinity;
  const maxDistance = 2; // Allow up to 2 character differences

  for (const word of words) {
    // Skip very short words (less than 3 chars)
    if (word.length < 3) continue;

    for (const location of LOCATION_NAMES) {
      const distance = levenshteinDistance(word, location);
      
      // Match if distance is within threshold and better than current best
      if (distance <= maxDistance && distance < bestDistance) {
        bestMatch = location;
        bestDistance = distance;
      }
    }
  }

  return bestMatch;
};

// Helper: Remove location keywords from query to get service query
export const removeLocationFromQuery = (query: string): string => {
  let cleanQuery = query.toLowerCase();
  const words = cleanQuery.split(/\s+/);
  
  // Filter out location-like words in a single pass
  const filteredWords = words.filter((word) => {
    if (word.length < 3) return true; // Keep very short words
    
    // Check if word matches any location
    for (const location of LOCATION_NAMES) {
      if (levenshteinDistance(word, location) <= 2) {
        return false; // Remove location match
      }
    }
    return true; // Keep non-location words
  });

  return filteredWords.join(" ").trim().replace(/\s+/g, " ");
};

// Helper: Build location names list from locations string - returns all main service locations
export const getLocationNamesFromLocations = (locationString: string): string[] => {
  const hasAnyLocation = LOCATION_NAMES.some((loc) =>
    locationString.toLowerCase().includes(loc)
  );

  // If any location is found, return all main service locations for consistency
  if (hasAnyLocation) {
    return ["Gurugram", "Delhi NCR", "Noida"];
  }

  return [];
};

// Backward compatibility alias
export const getStatesFromLocations = getLocationNamesFromLocations;

// Helper: Check if location matches (consolidated logic for reuse)
const locationMatches = (serviceLocations: string, targetLocation: string): boolean => {
  const normalized = targetLocation.toLowerCase();
  
  // Direct match
  if (serviceLocations.includes(normalized)) {
    return true;
  }
  
  // Support common aliases for the primary service locations.
  if (
    (normalized === "gurgaon" || normalized === "gurugram") &&
    serviceLocations.includes("gurugram")
  ) {
    return true;
  }

  if (
    (normalized === "delhi" || normalized === "delhi-ncr" || normalized === "ncr") &&
    serviceLocations.includes("delhi ncr")
  ) {
    return true;
  }

  if (
    (normalized === "greater-noida" || normalized === "greater noida") &&
    serviceLocations.includes("noida")
  ) {
    return true;
  }
  
  return false;
};

// Helper: Get all services available in a location
export const getServicesByLocation = (location: string): SearchDataItem[] => {
  return SEARCH_DATA.filter(item => locationMatches(item.locations.toLowerCase(), location));
};

// Helper: Resolve high-intent query aliases that should map to a specific existing service.
export const getSpecialSearchResults = (
  query: string,
  location?: string
): SearchDataItem[] => {
  const normalized = query.toLowerCase().trim();

  // "cricket turf installation/construction" and bare "turf" → terrace cricket
  if (
    /\bturf\b/.test(normalized) ||
    /cricket.*(turf|construction|installation)|turf.*(cricket|nets?)/.test(normalized)
  ) {
    const terraceCricketService = SEARCH_DATA.find((item) => item.id === "terrace-cricket");
    if (!terraceCricketService) return [];
    if (location && !locationMatches(terraceCricketService.locations.toLowerCase(), location)) return [];
    return [terraceCricketService];
  }

  // "cricket nets" (without practice/terrace/box/turf) → cricket-practice
  if (/\bcricket\s+nets?\b/.test(normalized) && !/practice|terrace|box|turf/.test(normalized)) {
    const cricketPracticeService = SEARCH_DATA.find((item) => item.id === "cricket-practice");
    if (!cricketPracticeService) return [];
    if (location && !locationMatches(cricketPracticeService.locations.toLowerCase(), location)) return [];
    return [cricketPracticeService];
  }

  // "hanger" / "hangers" queries should map to cloth-drying service
  if (/\bhang(?:er|ers)\b/.test(normalized) || /cloth\s*hang(?:er|ers)/.test(normalized) || /balcony\s*hang(?:er|ers)/.test(normalized)) {
    const clothService = SEARCH_DATA.find((item) => item.id === "cloth-drying");
    if (!clothService) return [];
    if (location && !locationMatches(clothService.locations.toLowerCase(), location)) return [];
    return [clothService];
  }

  return [];
};

// Helper: Get all services by category in a location
export const getServicesByCategoryAndLocation = (
  category: string,
  location: string
): SearchDataItem[] => {
  const normCat = category.toLowerCase();

  return SEARCH_DATA.filter((item) => {
    // Category match
    const catMatch = item.category.toLowerCase().includes(normCat) ||
                    normCat.includes(item.category.toLowerCase()) ||
                    item.keywords.toLowerCase().includes(normCat);
    
    // Location match using consolidated logic
    const locMatch = locationMatches(item.locations.toLowerCase(), location);
    
    return catMatch && locMatch;
  });
};

// Helper: Determine query type
export const determineQueryType = (
  query: string,
  detectedLocation: string | null
): "location-only" | "service-only" | "service+location" => {
  const cleanQuery = removeLocationFromQuery(query).trim();

  if (detectedLocation && cleanQuery.length === 0) {
    return "location-only";
  }

  if (detectedLocation && cleanQuery.length > 0) {
    return "service+location";
  }

  return "service-only";
};

// Helper: Build display name based on query context
export const buildDisplayName = (
  item: SearchDataItem,
  queryType?: "location-only" | "service-only" | "service+location",
  location?: string
): string => {
  // If location-only or service+location search, show "Service in Location"
  if (
    (queryType === "location-only" || queryType === "service+location") &&
    location
  ) {
    const locationNames: Record<string, string> = {
      gurugram: "Gurugram",
      "delhi-ncr": "Delhi NCR",
      noida: "Noida",
    };

    const locationName =
      locationNames[location] ||
      location.charAt(0).toUpperCase() + location.slice(1);

    return `${item.name} in ${locationName}`;
  }

  // Default: just service name (for service-only searches)
  return item.name;
};

// Helper: Build correct URL based on search result and location
export const buildSearchResultUrl = (
  item: SearchDataItem,
  queryType?: "location-only" | "service-only" | "service+location",
  location?: string
): string => {
  // Location-only search or service+location: navigate to service+location nested route
  if (
    (queryType === "location-only" || queryType === "service+location") &&
    location
  ) {
    return `/services/${item.id}/${location}`;
  }

  // Service-only: use item's own URL (which is /services/[slug])
  return item.url;
};
